...
public class StickHeroGame extends Application implements Serializable{
    ...
    public void start(Stage primaryStage) {
        ...
        playSound("./backgroundMusic.wav", 0.1f);
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
                if(isGameRunning) {
                    ...
                    gc.setFill(Color.BLACK);
                    gc.setFont(Font.font(20));
                    gc.fillText("Click SpaceBar = Flip/Unflip", 10, 70);

                    gc.setFill(Color.BLACK);
                    gc.setFont(Font.font(20));
                    gc.fillText("Hold Mouse Left Click = Extend Bridge", 10, 90);

                    gc.setFill(Color.BLACK);
                    gc.setFont(Font.font(20));
                    gc.fillText("Press S = Save Current Score, Highest Score and Total Cherries", 10, 110);

                    gc.setFill(Color.BLACK);
                    gc.setFont(Font.font(20));
                    gc.fillText("Press R = Restore saved Score, Highest Score and Total Cherries", 10, 130);

                    gc.setFill(Color.BLACK);
                    gc.setFont(Font.font(20));
                    gc.fillText("Automatic Revival if player has >=3 Cherries", 10, 150);
                    ...
                    if (isGameOver) {
                        // Revive Cherry's collected >3 then revive
                        if(cherriesCollected>=3){
                            cherriesCollected-=3;
                            isGameOver=false;
                            player.setX(lastplatformmiddle);
                            bridges.clear();
                        }
                        else {
                            displayEndScreen();
                    ...
                    for (Bridge bridge : bridges) {
                        ...
                        if (player.isFlipped && cT) {
                            for (int[] platformRange : platformEnds) {
                                if (player.getX() > platformRange[1]) {
...
//            }
    ...
    public static class EndScreen extends Pane {
        public EndScreen(int score,int cherriesCollected,int highestScore, Runnable onRestartCallback) {
            ...
            restartButton.setStyle("-fx-font-size: 24; -fx-background-color: #FF0000;");
            restartButton.setOnAction(event -> onRestartCallback.run());
            restartButton.setLayoutX(((WIDTH - restartButton.getWidth()) / 2)-100);
